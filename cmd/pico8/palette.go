...
var Pico8RGBAPalette = color.Palette{
...
func OptimumPalette(img image.Image) (color.Palette, []int) {
	...
	for i := 0; i < 16; i += 1 {
		str := indices[i]
		index := hist[str]
		color := Pico8RGBAPalette[index]
		optPal = append(optPal, color)
...
func CustomPalette(pico8Pal []int) (color.Palette, []int) {

	customPal := []color.Color{}
	...
	for i := 0; i < 16; i += 1 {
		...
		customPal = append(customPal, Pico8RGBAPalette[idx])
	}

	return customPal, pico8Pal
}
